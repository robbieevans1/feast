...
const access_token =
	...
	Promise.all([
		fetch(activityUrl, getWithAuth)
				.then(jsonFromResponse),
		fetch(foodUrl, getWithAuth)
				.then(jsonFromResponse)
	])
		.then(([data, foodData]) => {
    ...
    // add resting heart rate data to html
		...
		document.querySelector("#heart-rate").innerHTML = restingHeartRate;
				...
				const caloriesAvailable = `<p> Calories Available<span class="text-warning"> Right Now</span>: ${caloriesBurned - caloriesEaten - calorieDeficit}`
